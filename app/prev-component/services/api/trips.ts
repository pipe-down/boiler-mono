...
export async function getTrips(params?: { page?: number; size?: number; keyword?: string }) {
  ...
  const { data } = await api.get<ApiPage<ApiTrip>>(url, {
    params: {
...
export async function getMyTrips(params?: {
  ...
  const { data } = await api.get<ApiPage<ApiTrip>>('/trips/my', {
    params: {
...
export async function getTripById(tripId: string | number) {
  try {
    const { data } = await api.get<ApiResponse<ApiTrip>>(`/trips/${tripId}` as any);
    return data.data;
  } catch (e: any) {
    ...
    if (
      ...
      const { data } = await api.get<ApiResponse<ApiTrip>>(`/trips/public/${tripId}` as any);
      return data.data;
    }
    throw e;
  }
...
export async function joinTripByInviteCode(inviteCode: string, nickname?: string) {
  ...
  if (nickname) params.set('nickname', nickname);
  const { data } = await api.post<ApiResponse<any>>(`/trips/join?${params.toString()}`);
  return data;
}

export async function validateInviteCode(inviteCode: string) {
  const { data } = await api.get<
    ApiResponse<{ valid: boolean; tripId?: number; tripTitle?: string; creatorName?: string }>
  >(`/trips/invite-code/${inviteCode}/validate`);
  return data.data;
}

// Members/roles
export async function getTripMembers(tripId: string | number) {
  const { data } = await api.get<ApiResponse<any[]>>(`/trips/${tripId}/members` as any);
  return data.data || [];
}
...
export async function removeMember(tripId: string | number, targetUserId: string | number) {
  const { data } = await api.delete<ApiResponse<any>>(`/trips/${tripId}/members/${targetUserId}`);
  return data;
}
...
export async function updateTripStatus(tripId: string | number, status: string) {
  ...
  const { data } = await api.put<ApiResponse<any>>(`/trips/${tripId}/status?${params.toString()}`);
  return data;
}

export async function regenerateInviteCode(tripId: string | number) {
  const { data } = await api.post<ApiResponse<any>>(`/trips/${tripId}/invite-code/regenerate`);
  return data;
}

export async function getInviteCode(tripId: string | number) {
  const { data } = await api.get<ApiResponse<any>>(`/trips/${tripId}/invite-code`);
  return data?.data ?? data;
}

export async function leaveTrip(tripId: string | number) {
  const { data } = await api.delete<ApiResponse<any>>(`/trips/${tripId}/leave`);
  return data;
}
