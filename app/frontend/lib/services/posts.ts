...
export async function getPosts(params?: {
  ...
  const raw: any = await api.get('posts', {
    searchParams: { page, size, keyword: keyword || undefined, type: type || undefined },
  }).json();
...
export async function updatePost(
  ...
  if (input?.newFiles?.length || input?.deletedFileIds?.length) {
    ...
    const { data } = await api.put(`posts/${postId}`, { body: form });
    return data && typeof data === 'object' && 'data' in data ? (data as any).data : data;
  }
